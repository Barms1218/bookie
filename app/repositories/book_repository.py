...
async def search_books_local(db: AsyncSession, term: str):
    ...
    return result.scalars().all() # Extract the Book models from the rows
...
async def save_book_to_db(db: AsyncSession, book_schema: BookIngestSchema):
    ...
    stmt = insert(Book).values(
        ...
        extra_metadata=book_schema.metadata.model_dump()
