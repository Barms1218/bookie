...
async def add_user_book(schema: schemas.UserBookIngest, service: BookService = Depends(get_book_service)):
...
@router.post("/new", response_model=schemas.EntryPublic, status_code=201)
async def new_note(schema: schemas.EntryIngestSchema, service: BookService = Depends(get_book_service)):
